...
- Editor workspace: full-viewport layout — project sidebar on the left, center canvas, slide-over AI sidebar on the right.
- Project sidebar: `lg:` and up, opening it pushes the canvas right (the canvas area is never covered); below `lg`, it's a floating overlay with a dark semi-transparent backdrop instead, since pushing ~288px of content on a narrow viewport would crowd the canvas.
